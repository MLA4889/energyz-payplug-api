...
def _target_files_col() -> str:
    """
    Priorité à une colonne Files unique si tu en as configuré une (DOC_FILES_COLUMN_ID côté config.py),
    sinon fallback éventuels (INVOICE_FILES_COLUMN_ID puis QUOTE_FILES_COLUMN_ID).
    """
    # Si tu veux forcer une seule colonne pour tout, mets-la dans config.DOC_FILES_COLUMN_ID
    # et conserve le fallback ci-dessous.
    return getattr(settings, "DOC_FILES_COLUMN_ID", "") or settings.INVOICE_FILES_COLUMN_ID or settings.QUOTE_FILES_COLUMN_ID
...
def _iban_from_item(cols: Dict[str, Any]) -> str:
    """
    Récupère l’IBAN affiché par ta colonne *formule* d’IBAN si elle est configurée.
    Si vide, renvoie "" (les clés PayPlug utiliseront éventuellement AUTRE_IBAN si prévue).
    """
    if settings.IBAN_FORMULA_COLUMN_ID:
        txt = get_formula_display_value(cols, settings.IBAN_FORMULA_COLUMN_ID) or ""
        return txt.strip()
    return ""


# ----------------------------
# Endpoints de base / debug
# ----------------------------
@app.get("/")
def root():
...
def debug_payplug(item_id: int):
    ...
    return {
        ...
        "hint": "Si ta colonne IBAN (formule) est vide, ajoute une valeur ou configure AUTRE_IBAN dans PAYPLUG_KEYS_*_JSON."
    }
...
async def pay_acompte(n: int, item_id: Optional[int] = None, request: Request = None):
    """
    CORRECTION FastAPI : on ne tape PAS `Request | None` dans la signature,
    on garde `Request` (avec un default possible) pour éviter l'erreur
    'Invalid args for response field: Request | None ...'.
    """
    # Si POST: webhook Monday → on lit l’item_id du payload
    ...
    # Montant d’acompte lu via la colonne FORMULE correspondante (clé "1" ou "2")
    formula_col_id = settings.FORMULA_COLUMN_IDS.get(str(n))
    if not formula_col_id:
        raise HTTPException(400, f"FORMULA_COLUMN_IDS_JSON ne contient pas la clé '{n}'")
    ...
    if amount_cents <= 0:
        raise HTTPException(
            400,
            f"Montant d'acompte {n} invalide pour l'item {item_id} (valeur lue='{amount_txt or '0'}'). "
            "Mets un nombre > 0 dans la colonne formule."
        )
...
async def quote_from_monday_webhook(payload: dict = Body(...)):
    ...
    q = create_quote(token, client_id, {
        "description": desc,
        "amount_ht": amount_ht,
        "vat_rate": vat_rate,
    })
