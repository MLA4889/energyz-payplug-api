...
app = FastAPI(title="Energyz PayPlug API", version="2.2 (notification_url + webhook-only-status)")
...
async def quote_from_monday(request: Request):
    try:
        ...
        if not iban:
            ...
            merged = {**default_map, **{(k or "").strip().lower(): v for k, v in env_map.items()}}
            bl = (business_label or "").strip().lower()
            chosen = ""
            for k, v in merged.items():
                if not v:
                    continue
                if bl == k or bl.startswith(k) or (k in bl):
                    chosen = v.strip(); break
            logger.info(f"[IBAN] business_label='{business_label}' → chosen='{chosen}'")
            if chosen:
                iban = chosen
        ...
        # Écrit le lien dans la colonne de lien correspondante
        set_link_in_column(item_id, link_columns[acompte_num], payment_url, f"Payer acompte {acompte_num}")

        # ⚠️ NE PAS marquer “Payé …” ici. Le statut sera mis à jour par le webhook PayPlug.
