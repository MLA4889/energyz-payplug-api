...
app = FastAPI(title="Energyz PayPlug API", version="3.1 (no-form, webhook-paid-only)")
...
async def quote_from_monday(request: Request):
    try:
        ...
        payment_url = create_payment(
            ...
            email=email,                 # peut être vide -> fallback dans payments.py
            address=address,
        ...
        # Lien + statut “envoyé” (ne PAS mettre “Payé …” ici)
        m.set_link_in_column(item_id, link_columns[acompte_num], payment_url, f"Payer acompte {acompte_num} — {item_name}")
        status_after_create = _safe_json_loads(getattr(settings, "STATUS_ON_CREATE_JSON", None), default={}) or {}
        next_status = status_after_create.get(acompte_num, f"Lien acompte {acompte_num} envoyé")
...
async def payplug_webhook(request: Request):
    try:
        ...
        # Robust parsing (metadata peut arriver à divers endroits / formats)
        def _as_dict(x):
