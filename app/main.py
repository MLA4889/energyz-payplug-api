...
def _target_files_col() -> str:
    """Utilise une seule colonne Files prioritaire (DOC_FILES_COLUMN_ID),
    avec fallback éventuels si laissée vide."""
...
def _label_from_payload(payload: dict) -> str:
    # webhooks Monday renvoient parfois value="Label" ou value={"label":{"text":"Label"}}
    try:
        return payload["event"]["value"]["label"]["text"]
    except Exception:
        return str(payload.get("event", {}).get("value", "") or "")
...
# Petit debug PayPlug (utile pour voir l’IBAN lu et les clés en env)
...
async def pay_acompte(n: int, item_id: Optional[int] = None, request: Request = None):
    ...
    # Montant d'acompte depuis FORMULA_COLUMN_IDS[n]
    ...
    # Description + IBAN d’aiguillage
    desc = (settings.DESCRIPTION_COLUMN_ID and get_formula_display_value(cols, settings.DESCRIPTION_COLUMN_ID)) or info["name"]
    iban_txt = (settings.IBAN_FORMULA_COLUMN_ID and get_formula_display_value(cols, settings.IBAN_FORMULA_COLUMN_ID)) or ""

    pay = create_payment(amount_cents, f"Acompte {n} – {desc}", f"{settings.PUBLIC_BASE_URL}/", iban_txt)

    # Écrit le lien PayPlug dans la colonne Link si configurée
...
async def quote_from_monday_webhook(payload: dict = Body(...)):
    ...
    # On ne réagit que si le label correspond à QUOTE_TRIGGER_LABEL
...
# ----------------- Factures (acompte 1 / acompte 2 / solde) -----------------
...
async def invoice_from_monday(payload: dict = Body(...)):
    ...
    # Lien facture (si tu as des colonnes Link dédiées)
